...
//图片处理
const (
	METHOD_THUMBNAIL_SHORT_CROP_CENTER int = 11 //按短边缩放，居中裁剪
	METHOD_THUMBNAIL_LONG_FILL_SHORT   int = 12 //按长边缩放，缩略填充
	METHOD_FIXED_WIDTH_AUTO_HEIGHT     int = 21 //固定宽度，高度自适应
	METHOD_FIXED_HEIGHT_AUTO_WIDTH     int = 22 //固定高度，宽度自适应
)
...
//is_zoom:是否允许图片放大
func Proccess(img image.Image, width, height, method int, is_zoom bool) image.Image {
	//获取需要处理的图片的原图宽高函数
	...
	switch method {
	case METHOD_THUMBNAIL_SHORT_CROP_CENTER:
		//禁止图片放大
		if is_zoom == false && (width > w || height > h) {
	...
	case METHOD_FIXED_WIDTH_AUTO_HEIGHT:
		//禁止图片放大
		if is_zoom == false && width > w {
			width = w
		}
		img = imaging.Resize(img, width, 0, imaging.Linear)
	case METHOD_FIXED_HEIGHT_AUTO_WIDTH:
		//禁止图片放大
		if is_zoom == false && height > h {
			height = h
		}
